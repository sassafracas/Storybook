          ...
          centered={true}>
            <Modal.Content>
              <Carousel infiniteLoop useKeyboardArrows dynamicHeight>
                {this.props.history.location.state.photos.map(photo => {return <div key={photo.id}><img src={`https://stories-backend.herokuapp.com${photo.picture.url}`} alt=''/> <p className="legend">{photo.caption}</p></div>})}
              </Carousel>
            </Modal.Content>
        </Modal>
